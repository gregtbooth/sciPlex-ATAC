...
pMat = getMatrixFromArrow(
  ArrowFile = paste0(out_dir, "MLR_drug_filtered_annotated/ArrowFiles/MLR_5um_homeTn5.arrow"),
  ArchRProj = prj, 
...
cds = cds[,colData(cds)$Relative_Dose == 0]
...
root_nodes =  
  ordering_summary %>% 
  filter(cellType_broad == "Naive_Tcell" & fraction_cellType > 0.9)
...
saveRDS(cds, file = "cds_VehicleTreatedOnly_NaiveActive_Tcells_PT")
...
#cds = readRDS(file = "cds_VehicleTreatedOnly_NaiveActive_Tcells_PT")
...
pdf("Naive_to_Activated_Tcells_pseudotime_umap.pdf", width = 1.75, height = 1.75)
colData(cds) %>%
  as.data.frame() %>%
...
###############################################
cat( "plotting pseudotime trajectory ridges for each dose faceted by Drug\n")
combined.df = cdat %>% 
  dplyr::select(Stimulator, Pseudotime, Pseudotime_bin, cellType_broad, Drug, Relative_Dose) %>% 
  mutate(Stimulator = recode(Stimulator, StimA = "Allo", StimD = "Auto")) 

drugs = distinct(cdat, Drug)$Drug

for(d in drugs){
  pdf(paste0("PseudotimeRidges_Tcells_byDose_",d, ".pdf"), height = 2 , width = 4)
  print(
combined.df %>%
  filter(is.finite(Pseudotime), Drug == d) %>%
  ggplot() +
  geom_density_ridges(aes( x = Pseudotime, y = Relative_Dose, fill = Relative_Dose), size = .15) + 
  monocle3:::monocle_theme_opts() +
  facet_wrap(~Stimulator,ncol = 3) +
  theme(legend.position = "none", 
        text = element_text(size = 6),
...
# Plot pseudotime UMAPs for each drug, faceted by dose
for(d in drugs){
  pdf(paste0("UMAP_Pseudotime_Tcells_byDose_",d, ".pdf"), height = 2.5 , width = 2.5)
  print(
    cdat %>%
      filter(Drug == d) %>%
      ggplot(aes( x = UMAP1, y =  UMAP2, color = Pseudotime))+
      geom_point_rast(size=0.75, stroke = 0) +
      facet_wrap(~Relative_Dose) +
      monocle3:::monocle_theme_opts() +
      #theme_void() +
      #theme(legend.position = "right", text = element_text(size = 8), legend.key.width = unit(0.4,"line"), legend.key.height = unit(0.5,"line")) + 
      theme(legend.position = "none") +
...
get_binned_DA_sites_spline = function(drug, outdir = paste0(out_dir, "results/NB8/")){
