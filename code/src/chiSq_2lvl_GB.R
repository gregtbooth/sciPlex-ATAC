...
#########################
# Stuff for automating this analysis
...
chisq_vs_background <- function(test_hash_matrix, hash_frequencies){
    ...
    if (length(hash_frequencies_nz) == 0){
      pvals = rep(1.0, nrows(test_hash_matrix))
      names(pvals) = row.names(test_hash_matrix)
    } else {
      print (dim(test_hash_matrix))
...
######################################
## Automated counting of Cell hash UMIs from fastq files
